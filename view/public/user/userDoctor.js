...
//Get doctor
$.getJSON('/user', function(doctor) {
  if(doctor.doctor_id) {

    //Render doc name
    $('.local-user').text('Hej ' + doctor.first_name)
    $('.local-user').attr('id', doctor.doctor_id)

    getBookings()

  } else if(doctor.expired){
    alert('Logga in igen')
    window.location.href = '../index.html'
  } else {
    console.log('Fel i userDoctor.js getDoctor')
  }
})
...
function getBookings() {
  ...
  $.getJSON('/doctorBookings', function(bookings) {
    ...
    if(bookings[0]) {
      ...
      console.log('Error userDoctor.js getBookings()')
    }
  })
}
...
function createRow(booking) {
  ...
  row.id = booking.patient.patient_id
...
$('.button_start').on('click', function() {
  ...
  const doctor_id = $('.local-user').attr('id')
  const patient_id = $('.activeRow').attr('id')
  ...
  if(!videoConnected && doctor_id && patient_id && time && date) {
    ...
    $.ajax({
      ...
      success: function(booking) {
        if(booking._id) {
          ...
          console.log('Error in userDoctor.js booking')
        }
      },
    ...
    console.log('Något värde saknas i button_start on click')
  }
})
...
$(document).on('webkitfullscreenchange mozfullscreenchange fullscreenchange', function(e) {
    ...
    if (document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement) {
      //fullscreen
      /*$('#local-media video').css('position', 'absolute')
      $('#local-media video').css('z-index', '2147483647')*/
    } else {
      //not fullscreen
      /*$('#local-media video').css('position', 'static')
      $('#local-media video').css('z-index', '0')*/
    }
})
